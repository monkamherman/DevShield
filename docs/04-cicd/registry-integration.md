...
Promotion is digest-based: build once, store one image in Harbor, then sign and promote the same `image@sha256:<digest>` later. Phase 08 verifies the Cosign identity before a future deployment policy authorizes production; this workflow does not yet authorize deployment.
